...
// 💡 2. [신규 로직] 실제 사진 뷰어 모사를 위한 이중 광학 처리 (입자감 + 그라데이션) - React Style 최적화 버전
const getTonerVisuals = (code, role, desc = '') => {
  ...
  // 2. 플롭(측면) 컬러 추출 및 펄 간섭색 반영
  let flopColor = '#1e293b'; // 기본 다크 플롭
  if (isSolid) {
      flopColor = faceColor; // 솔리드는 크게 변하지 않음
  } else {
  ...
  if (isSolid) {
      ...
      // 순수 CSS Radial Gradient를 통한 매크로 입자 효과 시뮬레이션
      macroStyle = {
...
export default function App() {
      ...
      if (foundToners.length > 0 && (isIncrease || isDecrease)) {
        ...
        foundToners.forEach(item => {
          ...
          if (tonerInfo) {
            ...
            // 💡 2. 터미널 항목 앞단에 듀얼 컬러칩 HTML 생성 주입
            const visuals = getTonerVisuals(finalKey, tonerInfo.role, tonerInfo.desc);
            
            // React style 객체를 인라인 css 문자열로 변환하는 헬퍼 함수
            const styleObjToCssStr = (styleObj) => {
              if(!styleObj) return "";
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col relative overflow-x-hidden lg:overflow-hidden">
      ...
      <div className="flex-1 p-4 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start h-auto lg:h-[calc(100vh-72px)] lg:overflow-hidden">
        ...
        <div className="lg:col-span-5 flex flex-col h-auto lg:h-full space-y-4">
          ...
          <div className="bg-white border border-slate-300 rounded-xl p-4 flex flex-col flex-1 shadow-xl overflow-hidden min-h-[400px] lg:min-h-0">
      ...
      {/* 💡 2. [신규] 개별 안료 정밀 분석 뷰어 모달 (책자 완벽 모사) */}
          ...
          <div className="fixed inset-0 bg-slate-900/85 z-[120] flex items-center justify-center backdrop-blur-sm animate-in fade-in duration-200">
             <div className="bg-white rounded-2xl w-[700px] max-w-[95%] shadow-2xl overflow-hidden border border-slate-700">
                ...
                <div className="p-6">
                   ...
                   {/* 💡 책자 모사 레이아웃: 입자 정밀 관찰 (좌) / 색상 트래블 (우) */}
                   <div className="flex gap-4">
                      <div className="flex-1">
                         ...
                         <div className="h-44 rounded-xl shadow-inner border border-slate-400 relative overflow-hidden" style={visuals.macroStyle}>
                         </div>
                      </div>
                      <div className="flex-[1.5]">
                         ...
                         <div className="h-44 rounded-xl shadow-inner border border-slate-400 relative overflow-hidden" style={visuals.smoothStyle}>
                         </div>
                      </div>
                   </div>
