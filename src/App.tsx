...
import { 
  Sliders, Trash2, Plus, Zap, Maximize, Lock, Unlock, Layers, BrainCircuit, RefreshCw, Sun, Droplet, Camera, X, ChevronRight
} from 'lucide-react';
...
const TONER_DB = {
  ...
  'WT 333': { role: '그라나다 래드', desc: '밝은 적색 조색제. 주로 솔리드 컬러에 사용함. 이펙트 컬러에서 특히 45 & 110도에 적색이 부족할 경우 소량 사용됨.' },
  ...
  'WT 353': { role: '트랜스루센트 마젠타 래드', desc: '저농 자주색 조색제. WT338의 저농 버전. (WT353 : WT338 = 5.68 : 1)' },
  ...
  'WT 392': { role: '매직 이펙트', desc: '관찰각도에 따라 색상변화가 큰 특수 펄 조색제. 15도는 맑은 녹색, 45도는 맑은 적색, 110도는 약하게 적색으로 변함.' },
  'WT 393': { role: '라이트 옐로우', desc: '약하게 녹색을 띠는 밝은 황색 조색제. WT327에 비해 녹색이 적음.' },
  'WT 1051': { role: '블랜딩 1051', desc: '블랜드인 첨가제, 블랜딩용 첨가제.' },
  'WT 1500': { role: '울트라 딥 블랙', desc: '가장 어두운 흑색 조색제. 염료를 함유하고 있어 실버 2%, 솔리드/펄 최대 5% 이내 사용.' },
...
export default function App() {
  ...
  const handleClearAll = () => {
    ...
    setChatMessages([initialChat, { id: Date.now(), type: 'system', text: '🗑️ **[ACTION_RESET]** 모든 배합 리스트가 즉시 초기화되었습니다.', time: new Date().toLocaleTimeString('ko-KR') }]);
  };

  const handleConfirmBase = () => {
    setIsBaseConfirmed(true);
    setChatMessages(prev => [...prev, { id: Date.now(), type: 'system', text: '🔒 **[STATE_LOCK]** 기준 코드가 확정되었습니다. 멀티 시각화 레이어를 활성화합니다.', time: new Date().toLocaleTimeString('ko-KR') }]);
  };
  ...
  const handleAskSolution = () => {
    ...
    setTimeout(() => {
      ...
      setChatMessages(prev => [...prev, { id: Date.now(), type: 'ai', text: `👑 **[HI-TEC 마스터 엔진 V3.0]** 현장 조색 분석 명령어(${q}) 수신 완료. 실시간 보정 함수에 반영되었습니다.`, time: new Date().toLocaleTimeString('ko-KR') }]);
    }, 500);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800 font-sans flex flex-col relative overflow-x-hidden">
      ...
      {/* 📱 아이폰 및 PC 스크롤 반응형 레이아웃 핵심 전면 수정 */}
      <div className="flex-1 p-4 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start lg:overflow-hidden h-auto lg:h-[calc(100vh-72px)] overflow-y-auto">
        ...
        {/* Left Column: Editor (모바일 장치 높이 자동 해제) */}
        ...
        {/* Right Column: Multi-View & Engine Terminal (모바일에서 자연스럽게 위아래 결합) */}
