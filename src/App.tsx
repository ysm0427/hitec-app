...
import { 
  Sliders, Trash2, Plus, Zap, Maximize, Lock, Unlock, Layers, BrainCircuit, RefreshCw, Mic, FolderOpen, ChevronRight, Sun, Droplet, Camera, X, Image as ImageIcon
} from 'lucide-react';

// 💡 1. 안료 DB (설명글 잘림 방지 완벽 적용)
const TONER_DB = {
  ...
  'WT 317': { role: '플래틴 실버 브릴리언트 화인', desc: '반짝임이 좋은 매끄러운 특수 알루미늄 조색제. WT305 보다 15도는 밝고 나머지 각도는 어두움.' },
  ...
  'WT 322': { role: '마이크로 화이트', desc: '알루미늄 및 펄 입자가 사용되는 이펙트 컬러에만 사용함. 15도는 황색을 띠며 어둡고 나머지 각도는 청색을 띠며 밝게 함.' },
  ...
  'WT 326': { role: '그리니쉬 옐로우', desc: '이펙트 컬러에 사용하는 녹색을 띤 맑은 황색 조색제. 알루미늄 입자에 혼합하면 15도는 맑은 황색, 나머지 각도는 녹황색을 띰.' },
  'WT 327': { role: '옐로우', desc: '녹색을 띠는 밝은 황색 조색제. 주로 솔리드 컬러에 사용함.' },
  'WT 328': { role: '오커', desc: '주로 솔리드 컬러에 사용하는 탁한 황색.' },
  'WT 329': { role: '트랜스페어런트 옐로우', desc: '적색을 조금 띠는 선명하고 맑은 황색 조색제. 은폐력은 떨어짐.' },
  ...
  'WT 340': { role: '옐로우 마젠타 레드', desc: '맑은 자주색 조색제. 알루미늄 입자에 혼합할 경우 맑은 분홍색을 냄.' },
  ...
  'WT 390': { role: '플래틴 실버', desc: '중간 크기의 고휘도 광휘형 알루미늄 조색제. 15도가 가장 밝고 나머지 각도가 가장 어두움.' },
  ...
  'WT 1500': { role: '울트라 딥 블랙', desc: '가장 어두운 흑색 조색제. 알루미늄 입자에 2% 이상 사용시 주의.' },
  'WT 455': { role: '퍼포먼스 컴포넌트', desc: '솔리드 컬러에만 사용하는 첨가제. 작업성 및 외관 개선.' },
  'WT 3080': { role: '스페셜 애디티브', desc: '도막 보정 및 흐름 방지 첨가제' },
  'WT 6052': { role: '하이 퍼포먼스 클리어', desc: '바인더 및 투명 보정 용도.' },
};

const lerp = (a: number, b: number, t: number) => a + (b - a) * t;
const lerpHue = (a: number, b: number, t: number) => {
  let d = b - a; if (d > 180) d -= 360; if (d < -180) d += 360;
  let h = a + d * t; if (h < 0) h += 360; if (h >= 360) h -= 360;
  return h;
};
...
const getOptics = (tonersList: any[], weightKey: string) => {
  ...
  colorToners.forEach(t => {
    ...
    if (role.includes('컴포넌트') || role.includes('바인더') || role.includes('애디티브') || role.includes('클리어') || ['WT 385', 'WT 387', 'WT 386', 'WT 400', 'WT 3080', 'WT 310', 'WT 6052'].some(c => code.includes(c.replace('WT ', '')))) { wBinder += w; }
...
export default function App() {
  ...
  const handleCameraCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        addChatMessage('system', `📸 **[스마트 사진 뷰어 활성화]**\n현장의 손글씨 배합표는 웹 브라우저 단독으로 100% 자동 해독하기 어렵습니다.\n화면 상단에 고정된 사진을 직접 보시면서 아래 에디터에 코드를 빠르고 정확하게 타이핑해 주세요!`);
      };
  ...
  const handleAskSolution = () => {
    ...
    setTimeout(() => {
      ...
      addChatMessage('ai', `👑 **[명령 수신 완료]** 입력하신 (${q}) 내용을 확인했습니다. 실시간 광학 렌더링에 데이터를 즉각 반영합니다.`);
    }, 600);
  };
        ...
        <div className="bg-slate-900 border-b-4 border-blue-500 shadow-2xl z-50 p-2 md:p-4 sticky top-0 animate-in slide-in-from-top-10">
          <div className="flex justify-between items-center mb-2 px-2 max-w-[1600px] mx-auto">
            <h2 className="text-white text-sm md:text-base font-bold flex items-center">
              <ImageIcon className="mr-2 text-blue-400" size={18}/> 스마트폰 사진 고속 참조 모드
            </h2>
        ...
        <div className="lg:col-span-7 flex flex-col h-auto lg:h-full bg-white border border-slate-300 rounded-xl shadow-xl overflow-hidden mb-4 lg:mb-0">
          <div className="p-4 border-b border-slate-200 bg-slate-50 flex flex-col gap-4 shrink-0">
            <div className="flex items-center justify-between">
              ...
              <div className="flex space-x-2">
                ...
                <button onClick={() => cameraInputRef.current?.click()} className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded flex items-center text-[13px] font-bold transition-colors shadow-md">
                  <Camera size={16} className="mr-1.5" /> 시편 스캔
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <button onClick={handleConfirmBase} className="bg-slate-800 text-white px-4 py-2.5 rounded-md text-sm font-bold flex items-center whitespace-nowrap"><Lock size={14} className="mr-1"/>기준 확정</button>
                  ...
                  <div key={toner.id} className="flex flex-col bg-white p-3 mb-3 rounded-xl border border-slate-200 shadow-sm transition-all hover:border-blue-300">
                    <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-100">
                      <div className="flex items-center space-x-3 w-full">
                        <div className="w-12 h-6 rounded shadow-sm border border-slate-400 flex overflow-hidden cursor-pointer shrink-0" onClick={() => { if(TONER_DB[toner.code as keyof typeof TONER_DB]) setSelectedTonerForView(toner.code); }}>
                        ...
                        <input type="text" value={toner.code} onChange={(e) => handleCodeChange(toner.id, e.target.value, false)} placeholder="코드입력" className="flex-1 bg-transparent font-black text-blue-700 outline-none text-lg uppercase px-1" />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col space-y-3">
                      ...
                      <div className="w-full">
                        ...
                        <div className="text-[13px] text-slate-600 mt-2 leading-relaxed whitespace-pre-wrap break-keep">
                  ...
                  return (
                    <div key={toner.id} className="flex flex-col bg-white p-3 mb-3 rounded-xl border border-purple-200 shadow-sm transition-all hover:border-purple-400">
                      <div className="flex items-center justify-between mb-3 pb-2 border-b border-purple-50">
                        <div className="flex items-center space-x-3 w-full">
                          <div className="w-12 h-6 rounded shadow-sm border flex overflow-hidden cursor-pointer shrink-0" onClick={() => { if(TONER_DB[toner.code as keyof typeof TONER_DB]) setSelectedTonerForView(toner.code); }}>
                            <div className="flex-1" style={visuals.macroStyle}></div>
                            <div className="flex-1 border-l" style={visuals.smoothStyle}></div>
                          </div>
                          <input type="text" value={toner.code} onChange={(e) => handleCodeChange(toner.id, e.target.value, true)} placeholder="코드입력" className="flex-1 bg-transparent font-black text-purple-700 outline-none text-lg uppercase px-1" />
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col space-y-3">
                        <div className="w-full">
                          <div className="text-[15px] font-bold text-slate-800 leading-tight">{toner.role}</div>
                          <div className="text-[13px] text-slate-600 mt-2 leading-relaxed whitespace-pre-wrap break-keep">
                            {TONER_DB[toner.code as keyof typeof TONER_DB] ? TONER_DB[toner.code as keyof typeof TONER_DB].desc : '정확한 코드를 입력하면 정보가 표시됩니다.'}
                          </div>
                        </div>
